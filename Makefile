...
.PHONY: k8s-up k8s-down k8s-status logs-api pf-api pf-web migrate reset-db dev

k8s-up:
	kubectl apply -f k8s/00-namespace.yaml
	kubectl apply -f k8s/10-postgres.yaml
	kubectl apply -f k8s/11-api-secret.yaml
	kubectl apply -f k8s/12-api-config.yaml
...
k8s-down:
	-kubectl delete -f k8s/41-ingress-api.yaml --ignore-not-found=true
	-kubectl delete -f k8s/40-ingress-web.yaml --ignore-not-found=true
	-kubectl delete -f k8s/30-web.yaml --ignore-not-found=true
	-kubectl delete -f k8s/20-api.yaml --ignore-not-found=true
	-kubectl delete -f k8s/12-api-config.yaml --ignore-not-found=true
	-kubectl delete -f k8s/11-api-secret.yaml --ignore-not-found=true
...
dev: k8s-up
	...
	# Watch for changes in the api and web directories and redeploy on change
	kubectl apply -f k8s/40-ingress-web.yaml
	kubectl apply -f k8s/41-ingress-api.yaml